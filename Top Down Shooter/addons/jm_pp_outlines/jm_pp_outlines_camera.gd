@tool
class_name Outline_Camera extends Camera3D

var screen_mesh : MeshInstance3D;

var global_post_process_material : ShaderMaterial = preload("res://addons/jm_pp_outlines/post_process_outlines_material.tres");
var instance_post_process_material : ShaderMaterial = global_post_process_material.duplicate();

##Size of color outlines
#@export var color_outline_scale := 2.0: set = set_color_scale, get = get_color_scale
#
##Size of depth outlines
#@export var depth_outline_scale := 2.0: set = set_depth_scale, get = get_depth_scale
#
##Controls sensitivity to depth changes (lower values mean more outlines, but more artifacts too)
#@export_range(0,10) var depth_threshold := 10.0: set = set_depth_thres, get = get_depth_thres
#
##Multiplier for depth values
#@export var depth_multiplier := 1000.0: set = set_depth_multiplier, get = get_depth_multiplier
#
##General threshold for values to be considered as edges
#@export_range(0,1) var edge_threshold := 0.05: set = set_edge_threshold, get = get_edge_threshold
#
##Max edge alpha, lower values means edges blend more with background
#@export_range(0,1) var max_edge_alpha := 0.9: set = set_max_edge_alpha, get = get_max_edge_alpha
#
##General multiplier for edge alpha value, higher values mean harder edges
#@export var edge_alpha_multiplier := 3.0: set = set_alpha_multiplier, get = get_alpha_multiplier
#
##Outlines color
#@export var edge_color = Color(0,0,0,1): set = set_edge_color, get = get_edge_color
#
##Sets depth edges to use the laplace operator instead of sobel
#@export var background_tex : Texture: set = set_bg_tex, get = get_bg_tex
#
##Sets depth edges to use the laplace operator instead of sobel
#@export var depth_use_laplace := true: set = set_depth_use_laplace, get = get_depth_use_laplace
#
##Sets color edges to use the laplace operator instead of sobel
#@export var color_use_laplace := false: set = set_color_use_laplace, get = get_color_use_laplace
#
##Sets shader to use render the BG behind the edges
#@export var use_bg_texture := false: set = set_use_bg_texture, get = get_use_bg_texture
#
